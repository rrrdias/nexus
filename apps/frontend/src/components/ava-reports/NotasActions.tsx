...
export function NotasActions({ filters, institution }: { filters: any, institution?: string }) {
  ...
  const confirmSync = async () => {
    ...
    setSyncProgress(0)
    setSyncStep("Iniciando fila de sincronização de notas...")
    try {
      ...
      if (jobResult?.queued || res.data?.queued) {
        setSyncMessage(jobResult?.message || res.data?.message || "Solicitação de geração de notas enviada ao Moodle com sucesso. O relatório está sendo processado na fila do Moodle.")
      } else {
        setSyncMessage("As médias das notas e a média final foram sincronizadas e atualizadas com sucesso no sistema Nexus.")
      }
          ...
          {syncStatus === "success" && (
            <div className="flex flex-col items-center text-center py-4">
              ...
              <AlertDialogDescription className="text-gray-4 text-[14px] leading-relaxed max-w-sm mb-6">
                {syncMessage || "As médias das notas e a média final foram sincronizadas e atualizadas com sucesso no sistema Nexus."}
              </AlertDialogDescription>
              <AlertDialogAction 
                type="button"
                onClick={() => {
