const DEFAULT_PERIOD = '2026-2';

function splitCsv(value: string | undefined) {
  return (value || '')
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);
}

function getOrigin(value: string | undefined) {
  if (!value) return null;
  try {
    return new URL(value).origin;
  } catch {
    return null;
  }
}

function parseDate(value: string | undefined, fallback: string) {
  const source = value || fallback;
  const date = new Date(`${source}T00:00:00`);
  return isNaN(date.getTime()) ? new Date(`${fallback}T00:00:00`) : date;
}

export function getDefaultPeriod() {
  return process.env.NEXUS_DEFAULT_PERIOD || DEFAULT_PERIOD;
}

export function getAcademicPhaseDates() {
  return {
    fase1: {
      inicio: parseDate(process.env.NEXUS_FASE1_START, '2026-02-13'),
      fim: parseDate(process.env.NEXUS_FASE1_END, '2026-03-29'),
    },
    fase2: {
      inicio: parseDate(process.env.NEXUS_FASE2_START, '2026-03-30'),
      fim: parseDate(process.env.NEXUS_FASE2_END, '2026-05-11'),
    },
    fase3: {
      inicio: parseDate(process.env.NEXUS_FASE3_START, '2026-05-12'),
      fim: parseDate(process.env.NEXUS_FASE3_END, '2026-06-19'),
    },
  };
}

export function getLyceumActivePeriods() {
  const configured = splitCsv(process.env.LYCEUM_ACTIVE_PERIODS);
  if (configured.length > 0) return configured;

  const defaultPeriod = getDefaultPeriod();
  return defaultPeriod ? [defaultPeriod] : [];
}

export function getAllowedCorsOrigins() {
  const configured = splitCsv(process.env.CORS_ALLOWED_ORIGINS);
  const derived = [
    getOrigin(process.env.FRONTEND_URL),
    getOrigin(process.env.NEXT_PUBLIC_APP_URL),
    getOrigin(process.env.AUTH_URL),
    getOrigin(process.env.NEXT_PUBLIC_API_URL),
  ].filter((origin): origin is string => Boolean(origin));

  const defaults =
    process.env.NODE_ENV === 'production'
      ? []
      : [
          'http://localhost:3002',
          'http://127.0.0.1:3002',
          'http://localhost:3000',
          'http://127.0.0.1:3000',
        ];

  return Array.from(new Set([...configured, ...derived, ...defaults]));
}

export function isOriginAllowed(
  origin: string | undefined,
  allowedOrigins = getAllowedCorsOrigins(),
) {
  if (!origin) return true;

  return allowedOrigins.some((allowed) => {
    if (allowed === origin) return true;
    if (allowed.includes('*')) {
      const escaped = allowed
        .replace(/[.+?^${}()|[\]\\]/g, '\\$&')
        .replace(/\\\*/g, '.*');
      return new RegExp(`^${escaped}$`).test(origin);
    }
    return false;
  });
}
