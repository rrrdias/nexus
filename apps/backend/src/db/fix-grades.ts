...
async function fixDatabase() {
  console.log(
    'Iniciando correção do banco de dados para sincronização do Moodle...',
  );

  try {
    // 1. Apagar todos os dados de ava_grades_report (eles serão baixados novamente do Moodle sem perda de informações)
    console.log(
      '1. Limpando tabela ava_grades_report para remover duplicatas corrompidas...',
    );
    ...
    console.log(
      '2. Recriando a trava de segurança (Unique Constraint) no PostgreSQL...',
    );
    try {
      await client`
    ...
    console.log(
      'Banco de dados corrigido com sucesso! A sincronização de Notas agora funcionará.',
    );
    process.exit(0);
  } catch (error) {
