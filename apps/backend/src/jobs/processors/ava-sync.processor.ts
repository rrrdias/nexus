...
export class AvaSyncProcessor extends WorkerHost {
  ...
  async process(job: Job<AvaSyncJobData, any, string>): Promise<any> {
    ...
    this.logger.log(
      `Starting AVA Sync job (ID: ${job.id}, inst: ${institution || 'all'}, type: ${type || 'all'})`,
    );

    await job.updateProgress({
      progress: 5,
      step: 'Iniciando tarefas de sincronização do AVA...',
    });
    ...
    if (institution)
      tasksToProcess = tasksToProcess.filter(
        (t) => t.name === institution.toLowerCase(),
      );
    if (type)
      tasksToProcess = tasksToProcess.filter(
        (t) => t.type === type.toLowerCase(),
      );

    if (tasksToProcess.length === 0) {
      throw new Error(
        'Nenhuma tarefa de sincronização correspondente configurada.',
      );
    }
    ...
    for (let i = 0; i < total; i++) {
      ...
      const onProgress = async (subPercent: number, subStep: string) => {
        const overallProgress = Math.min(
          Math.round(taskBaseProgress + (subPercent / 100) * taskWeight),
          98,
        );
    ...
    await job.updateProgress({
      progress: 100,
      step: hasQueued
        ? 'Sincronização solicitada / em fila no Moodle'
