    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <Card className="border-t-4 border-[#1976D2] shadow-sm flex flex-col justify-between">
          <div>
            <CardHeader className="flex flex-row items-center justify-between pb-1">
              <CardTitle className="text-[10px] font-bold text-[#9AA0AC] uppercase tracking-wider">Progresso Médio Total</CardTitle>
              <button
                onClick={() => exportProgressData({ filters: currentFilters, type: 'general', title: `Progresso_Medio_${institution || 'Geral'}` })}
            ...
            <CardContent className="pt-2">
              <div className="flex items-center gap-3">
                <DonutChart percent={metrics.average_progress} color="#1976D2" label="PROG." />
                <div className="flex-1 space-y-0.5">
                  <PhaseBar 
                    label="Fase 01" 
                    avg={metrics.average_fase1} 
                    status={metrics.status_fase1} 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'general', phase: 1, title: `Progresso_Fase1_${institution || 'Geral'}` })}
                  />
                  <PhaseBar 
                    label="Fase 02" 
                    avg={metrics.average_fase2} 
                    status={metrics.status_fase2} 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'general', phase: 2, title: `Progresso_Fase2_${institution || 'Geral'}` })}
                  />
                  <PhaseBar 
                    label="Fase 03" 
                    avg={metrics.average_fase3} 
                    status={metrics.status_fase3} 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'general', phase: 3, title: `Progresso_Fase3_${institution || 'Geral'}` })}
                  />
                </div>
              </div>
            </CardContent>
        ...
        <Card className="border-t-4 border-amber-500 shadow-sm flex flex-col justify-between">
          <div>
            <CardHeader className="flex flex-row items-center justify-between pb-1">
              ...
              <button
                onClick={() => exportProgressData({ filters: currentFilters, type: 'below_expected', title: `Alunos_Abaixo_Esperado_${institution || 'Geral'}` })}
                className="text-[10px] font-bold text-[#9AA0AC] hover:text-navy border border-gray-200 hover:bg-gray-50 h-5 px-1.5 py-0.5 rounded flex items-center gap-1 transition-colors"
                title="Exportar dados do card"
              >
                <Download className="w-2.5 h-2.5" /> Exportar
            ...
            <CardContent className="pt-2">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 space-y-0.5">
                  <PhaseBar 
                    label="Fase 01" 
                    avg={metrics.f1_below > 0 ? Math.round((metrics.f1_below / metrics.total_records) * 100) : 0} 
                    status="danger" 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'below_expected', phase: 1, title: `Alunos_Abaixo_Esperado_Fase1_${institution || 'Geral'}` })}
                  />
                  <PhaseBar 
                    label="Fase 02" 
                    avg={metrics.f2_below > 0 ? Math.round((metrics.f2_below / metrics.total_records) * 100) : 0} 
                    status="danger" 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'below_expected', phase: 2, title: `Alunos_Abaixo_Esperado_Fase2_${institution || 'Geral'}` })}
                  />
                  <PhaseBar 
                    label="Fase 03" 
                    avg={metrics.f3_below > 0 ? Math.round((metrics.f3_below / metrics.total_records) * 100) : 0} 
                    status="danger" 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'below_expected', phase: 3, title: `Alunos_Abaixo_Esperado_Fase3_${institution || 'Geral'}` })}
                  />
                </div>
              </div>
            </CardContent>
        ...
        <Card className="border-t-4 border-red-500 shadow-sm flex flex-col justify-between">
          <div>
            <CardHeader className="flex flex-row items-center justify-between pb-1">
              <CardTitle className="text-[10px] font-bold text-[#9AA0AC] uppercase tracking-wider">Progresso Crítico</CardTitle>
              <button
                onClick={() => exportProgressData({ filters: currentFilters, type: 'critical', title: `Turmas_Criticas_${institution || 'Geral'}` })}
                className="text-[10px] font-bold text-[#9AA0AC] hover:text-navy border border-gray-200 hover:bg-gray-50 h-5 px-1.5 py-0.5 rounded flex items-center gap-1 transition-colors"
                title="Exportar dados do card"
              >
                <Download className="w-2.5 h-2.5" /> Exportar
            ...
            <CardContent className="pt-2">
              <div className="flex items-center gap-3">
                <DonutChart percent={critical_percent} color="#EF4444" label="CRÍTICO" />
                <div className="flex-1 space-y-0.5">
                  <PhaseBar 
                    label="Fase 01" 
                    avg={metrics.f1_crit > 0 ? Math.round((metrics.f1_crit / Math.max(metrics.total_disciplines, 1)) * 100) : 0} 
                    status="danger" 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'critical', phase: 1, title: `Turmas_Criticas_Fase1_${institution || 'Geral'}` })}
                  />
                  <PhaseBar 
                    label="Fase 02" 
                    avg={metrics.f2_crit > 0 ? Math.round((metrics.f2_crit / Math.max(metrics.total_disciplines, 1)) * 100) : 0} 
                    status="danger" 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'critical', phase: 2, title: `Turmas_Criticas_Fase2_${institution || 'Geral'}` })}
                  />
                  <PhaseBar 
                    label="Fase 03" 
                    avg={metrics.f3_crit > 0 ? Math.round((metrics.f3_crit / Math.max(metrics.total_disciplines, 1)) * 100) : 0} 
                    status="danger" 
                    onDownload={() => exportProgressData({ filters: currentFilters, type: 'critical', phase: 3, title: `Turmas_Criticas_Fase3_${institution || 'Geral'}` })}
                  />
                </div>
              </div>
            </CardContent>
        ...
        <Card className="border-t-4 border-navy shadow-sm flex flex-col justify-between">
          <div>
            <CardHeader className="flex flex-row items-center justify-between pb-1">
              ...
              <button
                onClick={() => exportProgressData({ filters: currentFilters, type: 'no_access', title: `Alunos_Sem_Acesso_${institution || 'Geral'}` })}
                className="text-[10px] font-bold text-[#9AA0AC] hover:text-navy border border-gray-200 hover:bg-gray-50 h-5 px-1.5 py-0.5 rounded flex items-center gap-1 transition-colors"
                title="Exportar dados do card"
              >
                <Download className="w-2.5 h-2.5" /> Exportar
